...
func categoryRiskDescription(category string) string {
	descriptions := map[string]string{
		...
		"ghes_server":         "Running an unsupported or outdated GHES version leaves the appliance exposed to unpatched vulnerabilities. GitHub only ships security patches for the most recent three releases, so version currency is the foundation of GHES security posture.",
		"ghes_authentication": "Built-in or password-based authentication on GHES bypasses centralized identity controls (SAML SSO / LDAP), making user lifecycle management, MFA enforcement, and offboarding inconsistent. External IdP integration is the standard control for enterprise GHES.",
		"ghes_networking":     "Disabled subdomain isolation, missing TLS enforcement, or unrestricted private mode expose the appliance to cross-site scripting, credential theft, and unauthenticated discovery. These are foundational network-layer controls that must be enabled in production.",
		"ghes_license":        "License expiration causes the appliance to enter a degraded read-only state; high seat utilization risks blocking new user provisioning. Monitoring both prevents avoidable operational incidents.",
		"ghes_infrastructure": "Misconfigured infrastructure settings (maintenance mode, admin SSH access, telemetry, signup) affect both the appliance's attack surface and its availability. These settings should match documented operational policy.",
	}
