...
func (s *EvaluationStage) Execute(ctx *ScanContext) error {
	...
	for key, data := range ctx.Results {
		...
		case strings.HasPrefix(key, "repository:"):
			...
			if !archived {
				switch {
				case repo.BranchProtection != nil && repo.BranchProtection.Protected:
				...
				case repo.RulesetProtection != nil && repo.RulesetProtection.Protected:
				...
				case repo.BranchProtection != nil:
		...
		case strings.HasPrefix(key, "ghes:"):
			...
			if ghes.Settings != nil {
				ctx.Results["evaluation:ghes_settings:"+hostname] = s.eval.EvaluateGHESSettings(ghes.ServerInfo, ghes.Settings, ghes.FeatureSupport)
			}
			...
			if ghes.SecurityAlerts != nil {
				ctx.Results["evaluation:ghes_security_alerts:"+hostname] = s.eval.EvaluateGHESSecurityAlerts(ghes.SecurityAlerts, ghes.FeatureSupport)
			}
